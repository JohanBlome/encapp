...
from encapp import run_cmd_silent


DEFAULT_TESTS = ['color_check_surfaces.json',
...
COLOR_RANGE = {COLOR_RANGE_UNKOWN:'unknown',
               COLOR_RANGE_FULL:'full',
               COLOR_RANGE_LIMITED:'limited'}

COLOR_STANDARD = {COLOR_STANDARD_BT709:'bt709',
                  COLOR_STANDARD_BT601_PAL: 'bt601_pal',
                  COLOR_STANDARD_BT601_NTSC: 'bt601_ntsc'}
COLOR_TRANSFER = {COLOR_TRANSFER_SDR_VIDEO: 'sdr'}
...
def check_color_values(path):
    ...
    ret, std_out, std_err = run_cmd_silent(cmd)
    ...
    reg_color_settings = '\(*(?P<range>[a-z]*)[ ,]*'\
                         ...
                         '(?P<color2>[a-z0-9]*)\/*'\
                         '(?P<color3>[a-z0-9]*)\)'
    match = re.search(reg_pix_format, std_out)
    pix_fmt = ""
    color1 = ""
    color2 = ""
    color3 = ""
    if match:
        ...
        match = re.search(reg_color_settings, match.group(0))
        if match:
            color1 = match.group('color1')
            color2 = match.group('color2')
            color3 = match.group('color3')

    single_pic_path = f'{path}_1st.png'
    cmd = f"ffmpeg -y -i {path} -vf 'scale=in_range=full:out_range=full' -vframes 1 {single_pic_path}"
    ...
    color_val_props(data, 'green', mediafile, img[0:h, section_width    :section_width * 2,:])
    color_val_props(data, 'blue',  mediafile, img[0:h, section_width * 2:section_width * 3,:])
    color_val_props(data, '0',     mediafile, img[0:h, section_width * 3:section_width * 4,:])
    color_val_props(data, '16',    mediafile, img[0:h, section_width * 4:section_width * 5,:])
    color_val_props(data, '127',   mediafile, img[0:h, section_width * 5:section_width * 6,:])
    color_val_props(data, '235',   mediafile, img[0:h, section_width * 6:section_width * 7,:])
    color_val_props(data, '255',   mediafile, img[0:h, section_width * 7:section_width * 8,:])
...
def check_media_file(filepath):
    ...
    result_str += '\nactual range:\n'            
    result_str += '{:3d}   {:3d}\n'.format(0, data.loc[(data['label'] == '0') & (data['plane'] == 'red')]['mean'].values[0])
    result_str += '{:3d}   {:3d}\n'.format(16, data.loc[(data['label'] == '16') & (data['plane'] == 'red')]['mean'].values[0])
    result_str += '{:3d}   {:3d}\n'.format(127, data.loc[(data['label'] == '127') & (data['plane'] == 'red')]['mean'].values[0])
    result_str += '{:3d}   {:3d}\n'.format(235, data.loc[(data['label'] == '235') & (data['plane'] == 'red')]['mean'].values[0])
    result_str += '{:3d}   {:3d}\n'.format(255, data.loc[(data['label'] == '255') & (data['plane'] == 'red')]['mean'].values[0])
    return result_str
...
def check_settings(resultpath):
    result_str = ""
    # ffprobe
    #Stream #0:0(eng): Video: h264 (Constrained Baseline) (avc1 / 0x31637661), yuv420p(tv, smpte170m/bt470bg/bt709), 
    ...
    for file in resultpath:
        with open(file) as resultfile:
            ...
            result_str += f'\n----- Verifying {jsonfile} -----\n'
            result_str += f'Encoder set to {COLOR_RANGE[int(color_range)]} color range'
