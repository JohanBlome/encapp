...
sample_config_json_data = [
    {
        ...
        KEY_NAME_I_INTERVALS: [2,10,60],
        KEY_NAME_DURATION: 10
    }
]
TEST_CLASS_NAME="com.facebook.encapp.CodecValidationInstrumentedTest"
JUNIT_RUNNER_NAME="com.facebook.encapp.test/android.support.test.runner.AndroidJUnitRunner"
ENCAPP_OUTPUT_FILE_NAME_RE = '.*_[0-9]+fps_[0-9]+x[0-9]+_[0-9]+bps_iint[0-9]+_m[0-9]+(?:\.mp4$|\.webm)'
...
def run_cmd(cmd):
    try:
        ...
        # print ('standard output: {}'.format(stdout.decode()))
        # print ('standard error: {}'.format(stderr.decode()))
    except:
        raise Exception('Failed to run adb command')
...
def check_device(serial_no):
    ...
    if len(device_serials) == 0:
        ...
        if (serial_no is None) :
            print ("More than one devices connected. Please specifiy a device serial number")
            exit(0)
...
class VideoAnalyzer:
    ...
    def get_bitrate(video_file):
        ...
        if video_file.endswith('.mp4'):
            ffprobe_cmd = 'ffprobe -v error -select_streams v:0 -show_entries stream=bit_rate -of default=noprint_wrappers=1:nokey=1 ' + video_file
        else:
            ffprobe_cmd = 'ffprobe -v error -show_entries format=bit_rate -of default=noprint_wrappers=1:nokey=1 ' + video_file
        stdout,stderr = run_cmd(ffprobe_cmd)
        if stdout == '':
    ...
    def get_resolution(mp4_file):
        ffprobe_cmd = 'ffprobe -v error -select_streams v:0 -show_entries stream=height,width -of csv=s=x:p=0 ' + mp4_file
        stdout,stderr = run_cmd(ffprobe_cmd)
        if stdout == '':
    ...
    def get_source_info(self,mp4_file_ref):
        ...
        if source_info is None:
            input_yuv_file = '/'.join([self.temp_dir,mp4_file_ref.strip(".mp4")+".yuv"])
            ffmpeg_cmd = 'ffmpeg -y -i ' + mp4_file_ref + ' -pix_fmt yuv420p -vsync 0 ' + input_yuv_file
            run_cmd(ffmpeg_cmd)
    ...
    def get_vmaf_score(self,mp4_file_ref, mp4_file_out):
        ...
        ffmpeg_cmd = 'ffmpeg -y -i ' + mp4_file_out + ' -pix_fmt yuv420p -vsync 0 -s ' + src_res + ' ' + output_yuv_file
        run_cmd(ffmpeg_cmd)

        sizes = src_res.split('x')
        vmaf_cmd = 'run_vmaf yuv420p ' + sizes[0] + ' ' + sizes[1] + ' ' + input_yuv_file + ' ' + output_yuv_file + ' --out-fmt json'
        stdout,stderr = run_cmd(vmaf_cmd)
        #remove the output yuv file
        os.system("rm " + output_yuv_file)
        vmaf_result = json.loads(stdout)

        if vmaf_result:
            if vmaf_result.get("aggregate") is not None and vmaf_result.get("aggregate").get("VMAF_score") is not None:
                vmaf_score = vmaf_result['aggregate']["VMAF_score"]
                return round(float(vmaf_score),2)
        return None

class JobInfo:
    def __init__(self, desc, in_file,output_dir, codec, res,mode,i_interval,br,duration,use_surface_enc):
        print("in_file:" + in_file, "codec:"+codec,"enc_resolution:"+res,"rc_mode:"+mode,"i_interval:"+i_interval,"bitrate:"+br,"duration:"+duration,sep=',')
        self.desc = desc
        ...
        self.src_res= res
        self.enc_res= res #encoding resolution of - [width]x[height] 1280x720
        ...
        self.extra_descr="dummy" #A description used to name the directory to place data
        self.use_surface_enc = use_surface_enc
        self.input_yuv_file = "ref.yuv"

    def dump_job_info(self, file_name):
        with open(file_name,'w') as fp:
            job_info_json = {
                "desc": self.desc
            }
            json.dump(job_info_json,fp, indent = 4)
            fp.close()
    ...
    def run_one_encode(self,job_info,serial_no):
        ...
        if job_info.input_file not in self.in_file_dict:
            ...
            if job_info.use_surface_enc:
                ...
                ffmpeg_cmd = 'ffmpeg -y -i ' + job_info.input_file + ' -s ' + job_info.enc_res + ' -pix_fmt nv12 ' + input_file
                run_cmd(ffmpeg_cmd)
            adb_cmd = 'adb -s ' + serial_no + ' push ' + input_file + ' ' + job_info.device_workdir + input_file
            run_cmd(adb_cmd)

        adb_cmd = 'adb -s ' + serial_no + ' shell am instrument -w -r'\
                   +' -e key ' + job_info.i_interval\
                   +' -e enc '+ job_info.codec\
                   +' -e file ' +job_info.device_workdir + (job_info.input_file if job_info.use_surface_enc else job_info.input_yuv_file)\
                   +' -e test_timeout 20'\
        ...
        for file in output_files:
            ...
            adb_cmd = 'adb -s ' + serial_no + ' pull /sdcard/' + file+' ' + job_info.output_dir
            run_cmd(adb_cmd)
            #remove the output
            adb_cmd = 'adb -s ' + serial_no + ' shell rm /sdcard/' + file
            run_cmd(adb_cmd)
    ...
    def get_rate_distortion(self):
        ...
        for group in self.job_info_groups:
            ...
            for job_info in group:
                ...
                score = video_analyzer.get_vmaf_score(job_info.input_file,job_info.output_file)
                bitrates.append(bitrate)
                vmaf_scores.append(score)

            if len(bitrates):
                job_info = group[0]
                group_result = {  "description": job_info.desc if job_info.desc is not None else "Test " + str(group_index),
                                "bitrates": bitrates,
...
def build_tests(tests_json,device_model):
    ...
    for test in tests_json:
        ...
        use_surface_enc = True if test.get(KEY_NAME_USE_SURFACE_ENC) == 1 else False
        job_info_group = []
                ...
                for res in enc_resolutions:
                    for mode in rc_modes:
                        for i_interval in i_intervals:
                            sub_dir = "_".join([in_file,codec,res,mode,str(i_interval)+'s'])
                            output_dir = workdir + '/' + sub_dir
                            os.system('mkdir -p ' + output_dir)
                            job_info_group = []

                            job_desc = group_desc if group_desc != "" else  "_".join([device_model, in_file.strip(".mp4"), codec,res,mode,'iint',str(i_interval)])
                            for br in bitrates:
                                job_info = JobInfo(job_desc,in_file, output_dir ,codec,res, mode, str(i_interval),str(br),str(duration),use_surface_enc)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='A Python script to run ENCAPP tests on Android and collect results. \
    The script will create a directory based on device model and date, and save encoded video and rate distortion results in the directory')
    parser.add_argument("--test",help="Test cases in JSON format.")
    parser.add_argument("--serial",help="Android device serial number")
    parser.add_argument("--config",help="Generate a sample config file in json format")
