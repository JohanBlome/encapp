        ...
        with open(inputfile) as json_file:
            ...
            try:
                ...
                if (len(decoded_data) > 0):
                    try:
                        decoded_data['codec'] = (
                                alldata['decoder_media_format']['mime'])
                    except Exception:
                    ...
                    try:
                        ...
                        # decoded_data['bitrate'] = (
                        #     alldata['settings']['bitrate'])
                        decoded_data['height'] = (
                            alldata['decoder_media_format']['height'])
                    except Exception:
                        print('Failed to read decoder data')
                    ...
                    decoded_data = (
                        decoded_data.loc[decoded_data['proctime'] >= 0])
            ...
            try:
                ...
                if (len(gpu_data) > 0):
                    ...
                    gpu_max_clock = int(
                        alldata['gpu_data']['gpu_max_clock'])
                    # print('codec = ' +
                    #   str(alldata['decoder_media_format']['mime']) +
                    #   ', mean = '  + str(mean/1000000))
                    gpuload_data = pd.DataFrame(
                        alldata['gpu_data']['gpu_load_percentage'])
                    gpuclock_data = pd.DataFrame(
                        alldata['gpu_data']['gpu_clock_freq'])
                    gpuclock_data['clock_perc'] = (
                        gpuclock_data['clock_MHz'].astype(float) * 100.0 /
                        gpu_max_clock)
                    plot_gpuprocessing(gpuload_data,
                                       gpuclock_data,
                                       gpu_model,
                                       options,
                                       gpu_max_clock)
