...
class TestAdbCommands(unittest.TestCase):
    ...
    def test_get_device_info_with_not_devices_shall_raise_error(
            self, mock_run):
    ...
    @unittest.mock.patch('encapp_tool.adb_cmds.run_cmd')
    def test_get_device_info_with_found_device_shall_return_info(
            self, mock_run):
    ...
    @unittest.mock.patch('encapp_tool.adb_cmds.run_cmd')
    def test_get_device_info_with_missing_device_shall_raise_exception(
            self, mock_run):
        mock_run.return_value = (True, ADB_DEVICES_MULTI_DEV, '')
        with self.assertRaises(AssertionError) as exc:
            encapp_tool.adb_cmds.get_device_info(
                'not_connected_device', debug=0)
        self.assertEqual(
            exc.exception.__str__(),
            'error: device not_connected_device not available'
        )
    ...
    @unittest.mock.patch('encapp_tool.adb_cmds.run_cmd')
    def test_remove_files_using_regex_shall_remove_regex_match_files(
            self, mock_run):
        ...
        mock_run.assert_has_calls(
            [
                ...
                unittest.mock.call(
                    f'adb -s {ADB_DEVICE_VALID_ID} shell '
                    'rm /sdcard/encapp_1.txt', 1),
                unittest.mock.call(
                    f'adb -s {ADB_DEVICE_VALID_ID} shell '
                    'rm /sdcard/encapp_2.txt', 1),
                unittest.mock.call(
                    f'adb -s {ADB_DEVICE_VALID_ID} shell '
                    'rm /sdcard/encapp_logs.log', 1
                ),
            ],
            any_order=True,
        )
    ...
    @unittest.mock.patch('encapp_tool.adb_cmds.run_cmd')
    def test_get_app_pid_inactive_process_shall_return_not_running_code(
            self, mock_run):
    ...
    @unittest.mock.patch('encapp_tool.adb_cmds.run_cmd')
    def test_get_app_pid_invalid_id_output_shall_return_error_code(
            self, mock_run):
    ...
    def test_grant_storage_permission_shall_grant_external_storage_with_adb(
        ...
        pm_grant_str = (f'adb -s {ADB_DEVICE_VALID_ID} shell '
                        'pm grant com.example.myapp ')
        encapp_tool.adb_cmds.grant_storage_permissions(
            ADB_DEVICE_VALID_ID, apk, 0)
        mock_run.assert_has_calls(
            [
                unittest.mock.call(
                    pm_grant_str +
    ...
    def test_force_stop_shall_use_am_to_stop_package(self, mock_run):
        apk = 'com.example.myapp'
        expected_cmd = (f'adb -s {ADB_DEVICE_VALID_ID} shell '
                        f'am force-stop {apk}')
