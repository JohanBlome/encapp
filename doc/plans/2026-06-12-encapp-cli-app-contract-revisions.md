...
$ python3 scripts/encapp.py run tests/bitrate_buffer.pbtxt -i /tmp/akiyo_qcif.y4m \
      --serial $ANDROID_SERIAL -w /tmp/encapp_readme_flow
...
```
$ /tmp/encapp_run_v2.sh -s $ANDROID_SERIAL -o /tmp/encapp_p10b_fix /tmp/encapp_smoke.pbtxt
...
| **`--serial <SERIAL>` CLI flag** | Selects which adb device to target (e.g. `--serial $ANDROID_SERIAL`). Threaded through every `adb -s <SERIAL> ...` call. | Untouched. |
...
```json
{
  "schema_version": 1,
  "session_id": "R...",
  "device": {"serial": "$ANDROID_SERIAL", "app_version": "1.31"},
  "total_tests": 1,
...
```
$ encapp run /tmp/encapp_smoke.pbtxt --serial $ANDROID_SERIAL -w /tmp/encapp_p8b
=== session R...: 1/1 tests passed ===
...
```
$ /Users/jblome/code/encapp/scripts/encapp_run.sh -s $ANDROID_SERIAL \
      -o /tmp/encapp_p6_bash /tmp/encapp_smoke.pbtxt
  Total:  1
  Passed: 1
  Failed: 0
$ adb -s $ANDROID_SERIAL shell ls /sdcard/ | grep session.jsonl
...
```
$ python3 scripts/encapp.py run /tmp/encapp_smoke.pbtxt -r 1M,2M,5M --serial $ANDROID_SERIAL -w /tmp/encapp_p5_sweep
=== session R...: 3/3 tests passed ===
...
**Surfaced by:** plan execution + on-device E2E testing (serial $ANDROID_SERIAL)
...
**Surfaced by:** smoke test on device (serial $ANDROID_SERIAL, Pixel 9, app v1.31)
